...
use crate::cp_amm_types::{
    Pool, Position, CreatePositionAccounts,
};
...
pub fn claim_position_fees_quote_only<'info>(
    ...
    let base_treasury_before = {
        let account = TokenAccount::try_deserialize(&mut base_treasury_ata.try_borrow_data()?.as_ref())?;
        account.amount
    };
    ...
    invoke_signed(
        &anchor_lang::solana_program::instruction::Instruction {
            ...
            accounts: [
                ...
                AccountMeta::new(base_treasury_ata.key(), false),

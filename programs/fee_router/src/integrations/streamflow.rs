...
pub fn parse_investor_accounts<'info>(
    ...
    require!(
        end_idx <= remaining_accounts.len(),
...
pub fn read_locked_amount_from_stream(
    ...
    let stream_contract = StreamflowContract::deserialize(&mut &stream_account.data.borrow()[..])
        .map_err(|_| HonouraryError::InsufficientStreamflowData)?;

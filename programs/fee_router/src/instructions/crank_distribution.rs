...
use crate::{
    ...
    utils::math::*,
    integrations::{
        cp_amm::claim_position_fees_quote_only,
    },
    events::*,
};
...
pub struct CrankDistribution<'info> {
    ...
    /// DAMM v2 pool
    #[account(
        constraint = pool.key() == position_owner.pool
    )]
    ...
    /// Quote mint
    #[account(
        constraint = quote_mint.key() == position_owner.quote_mint
    )]
    ...
    #[account(
        token::mint = position_owner.position_mint,
        token::authority = position_owner
    )]
...
pub fn handle_crank_distribution<'info>(
    ...
    require!(
        progress.can_distribute(current_time),
    ...
    let claimed_quote = if is_first_page && progress.day_completed {
        ...
        require!(
            total_locked_all_investors > 0,
            HonouraryError::MathOverflow
        );
    ...
    require!(
        end_idx <= ctx.remaining_accounts.len(),
        HonouraryError::InvalidPagination
    );
